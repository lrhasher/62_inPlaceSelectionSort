...
public class OrderedList_inArraySlots
    ...
    /**
      ...
      @return ??
     */
     private void champIndex(int startingIndex) {
         int currentValue, min , index , storage;
         for(index = startingIndex, min = get(startingIndex); index < size(); index++){
             currentValue = get(index);
             if(currentValue < min){
             storage = min;
             min = currentValue;
             list_iAS.set(startingIndex, min);
             list_iAS.set(index , storage);
             }
         }
     }
